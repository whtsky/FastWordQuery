    ...
    def _deco(cls):
        ...
        for method in methods:
            ...
            if attrs and attrs[1] == -1:
                exports.append((getattr(method[1], "__def_index__", 0), method[1]))
...
class Service:
    ...
    def _get_field(self, key, default=""):
        return (
            ...
            if self.cached(key)
            else self._get_from_api().get(key, default)
        )
    ...
    def download(cls, url, filename, timeout=15):
        ...
        try:
            with open(filename, "wb") as f:
                f.write(requests.get(url, headers={"User-Agent": _default_ua}).content)
            return True
        except Exception:
            pass
    ...
    def net_stream(
        ...
        for number, (url, params) in enumerate(targets, 1):
            ...
            response = urllib2.urlopen(
                urllib2.Request(
                    url=(
                        "?".join([url, params]) if params and method == "GET" else url
                    ),
                    headers=headers,
                ),
            ...
            if not response:
                raise OSError("No response for %s" % desc)

            if response.getcode() != 200:
                value_error = ValueError(
                    "Got %d status for %s" % (response.getcode(), desc)
                )
                try:
            ...
            if "mime" in require and require["mime"] != format(
...
class LocalService(Service):
    ...
    @property
    def title(self):
        return getattr(self, "__register_label__", "Unkown")
...
class MdxService(LocalService):
    ...
    def _get_default_html(self):
        ...
        if result:
            if result.upper().find("@@@LINK=") > -1:
                # redirect to a new word behind the equal symol.
                word = result[len("@@@LINK=") :].strip()
    ...
    def adapt_to_anki(self, html):
        ...
        html = p.sub("[sound:mdx-" + self.title + "-" + "\\1]\\2", html)
        self.save_media_files(media_files_set)
        for f in mcss:
            cssfile = "_{}".format(os.path.basename(f.replace("\\", os.path.sep)))
            ...
            if not os.path.exists(cssfile):
                css_src = self.dict_path.replace(self._filename + ".mdx", f)
            ...
            html = f'<div class="{wrap_class_name}">{html}</div>'
    ...
    def save_default_file(self, filepath_in_mdx, savepath=None):
        ...
        if os.path.exists(savepath):
            return savepath
        try:
            src_fn = self.dict_path.replace(self._filename + ".mdx", basename)
            if os.path.exists(src_fn):
                shutil.copy(src_fn, savepath)
                return savepath
            else:
                ignorecase = config.ignore_mdx_wordcase and (
                    filepath_in_mdx != filepath_in_mdx.lower()
                    or filepath_in_mdx != filepath_in_mdx.upper()
                )
...
class StardictService(LocalService):
    ...
    def __init__(self, dict_path):
        ...
        if StardictService.check(self.dict_path):
            ...
            self.builder = self._get_builer(
                dict_path, service_wrap(StardictBuilder, dict_path, in_memory=False)
            )
            # if self.builder:
