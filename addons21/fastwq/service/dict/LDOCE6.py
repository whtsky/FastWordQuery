...
VOICE_PATTERN_WQ = (
    r'<span class="%s"><a href="sound://([\w/]+\w*\.mp3)">(.*?)</span %s>'
)
MAPPINGS = [
    [
        "br",
        [
            re.compile(VOICE_PATTERN % r"r"),
            re.compile(VOICE_PATTERN_WQ % (r"brevoice", r"brevoice")),
        ],
    ],
    [
        "us",
        [
            re.compile(VOICE_PATTERN % r"b"),
            re.compile(VOICE_PATTERN_WQ % (r"amevoice", r"amevoice")),
        ],
    ],
]
...
@register(["本地词典-LDOCE6", "MDX-LDOCE6"])
class Ldoce6(MdxService):
    ...
    @export(["例句加音频", "Examples with audios"])
    ...
    def fld_definate(self):
        ...
        if m:
            ...
            if el_list:
                maps = [
                    "".join(str(content) for content in element.contents)
    ...
    @export(["随机例句加音频", "Random example with audio"])
    def fld_random_sentence_audio(self):
        return self._range_sentence_audio()

    @export(["首2个例句加音频", "First 2 examples with audios"])
