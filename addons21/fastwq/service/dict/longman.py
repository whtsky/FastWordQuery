...
class Longman(WebService):
    ...
    def _get_from_api(self):
        ...
        for dic_link in dictlinks:
            ...
            if am_s_tag:
                word_info["am_mp3"] = am_s_tag.get("data-src-mp3", "")
                am_s_tag.decompose()
            if br_s_tag:
                word_info["br_mp3"] = br_s_tag.get("data-src-mp3", "")
                br_s_tag.decompose()
            ...
            if head_tag and not head_finded:
                ...
                try:
                    pron_codes = "".join(
                        list(head_tag.find("span", {"class": "PronCodes"}).strings)
                    )  # Hyphenation
                except:
                    pron_codes = ""
                try:
