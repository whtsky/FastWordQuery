...
@register(["韦氏词典", "Merriam-Webster"])
class MerriamWebster(WebService):
    def __init__(self):
        super().__init__()

    def _get_from_api(self):
        url = "https://www.merriam-webster.com/dictionary/{}".format(
        ...
        for a in sym_div.findAll("a"):
            if "thesaurus" not in a["href"]:
                a["href"] = "https://www.merriam-webster.com/dictionary{}".format(
                    a["href"]
                )
            else:
    ...
    @export(["同义词反义词", "Synonyms & Antonyms"])
