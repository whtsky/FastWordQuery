...
class Dreye(WebService):
    ...
    def _get_from_api(self):
        data = self.get_response(
            "https://yun.dreye.com/dict_new/dict.php?w={}&hidden_codepage=01".format(
                self.quote_word
            )
        )
        soup = parse_html(data)
        result = {"phon": "", "pron": "", "pos": "", "def": ""}
        ...
        if mp3_match:
            result["pron"] = f"{mp3_match.group(1)}"
        # 动变
        element = soup.find("div", id="digest")
        if element:
            result["pos"] = f"{str(element)}"
        # 释义
        element = soup.find("div", id="usual")
        if element:
            result["def"] = f"{str(element)}"
