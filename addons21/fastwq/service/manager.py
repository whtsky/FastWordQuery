...
class ServiceManager:
    ...
    def update_services(self):
        (
            self.mdx_services,
            self.star_dict_services,
        ) = self._get_available_local_services()
        ...
        self.local_services = (
            self.mdx_services + self.star_dict_services + self.local_custom_services
        )
    ...
    def _get_services_from_files(self, *args):
        ...
        local_custom_services = sorted(
            local_custom_services, key=lambda service: service.__title__
        )
            ...
            for dirpath, dirnames, filenames in os.walk(each):
                for filename in filenames:
                    ...
                    if MdxService.check(dict_path):
                        ...
                        service.__unique__ = md5(
                            str(dict_path).encode("utf-8")
                        ).hexdigest()
